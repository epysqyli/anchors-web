...
const EventAnchor: Component<Props> = (props): JSX.Element => {
  const anchorNostrIDstyle = (): string => {
    if (useLocation().pathname.includes(props.nostrEventID)) {
      return "text-sm break-all w-1/2 md:w-1/6 text-slate-400 bg-slate-700 px-2 py-1 rounded cursor-default";
    }

    return `text-sm break-all md:w-1/5 text-slate-400 cursor-pointer bg-slate-700
